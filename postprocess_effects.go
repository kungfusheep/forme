...
func PPBloom(radius int, threshold, intensity float64) PostProcess {
	return func(buf *Buffer, ctx PostContext) {
		...
		// resolveFG would make every uncoloured cell appear as bright as the terminal's
		// default FG, flooding the luminance threshold and causing bloom everywhere.
...
// PPCRT simulates a CRT monitor — scanlines, vignette, and warm phosphor
...
// quantizeUint8 rounds v to the nearest multiple of step.
func quantizeUint8(v, step uint8) uint8 {
	if step <= 1 {
		return v
	}
	return uint8(math.Round(float64(v)/float64(step))) * step
}

// WithQuantize wraps a PostProcess with output quantization.
// Runs pp first, then snaps all resulting RGB values to the nearest multiple of step.
// Equivalent to chaining pp then PPQuantize(step), but composable in one expression.
// Use step=32 for animated effects like PPPlasma — reduces bytes/frame by ~40-50%
// with acceptable banding at typical terminal sizes.
func WithQuantize(step uint8, pp PostProcess) PostProcess {
	q := PPQuantize(step)
	return func(buf *Buffer, ctx PostContext) {
		pp(buf, ctx)
		q(buf, ctx)
...
// PPQuantize snaps all RGB colour channels to the nearest multiple of step.
// Reduces unique colours per frame, enabling the style optimizer to batch
// consecutive same-colour cells into a single escape sequence.
// step=8 gives 32 levels per channel (subtle), step=16 gives 16 (visible banding).
// Use as a final pass after animated effects to trade imperceptible quality for
// dramatically fewer bytes written to the terminal each frame.
func PPQuantize(step uint8) PostProcess {
	return EachCell(func(_, _ int, c Cell, _ PostContext) Cell {
		if c.Style.FG.Mode == ColorRGB {
			c.Style.FG.R = quantizeUint8(c.Style.FG.R, step)
			c.Style.FG.G = quantizeUint8(c.Style.FG.G, step)
			c.Style.FG.B = quantizeUint8(c.Style.FG.B, step)
		}
		if c.Style.BG.Mode == ColorRGB {
			c.Style.BG.R = quantizeUint8(c.Style.BG.R, step)
			c.Style.BG.G = quantizeUint8(c.Style.BG.G, step)
			c.Style.BG.B = quantizeUint8(c.Style.BG.B, step)
		}
		return c
	})
}
